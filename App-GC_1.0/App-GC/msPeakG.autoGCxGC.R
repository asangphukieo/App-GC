...
########################################
# Modified by: Apiwat Sangphukieo      #  
# Email: apiwat.bif@mail.kmutt.ac.th   #
# Version: v.1                         #
# Date: April. 20, 2020                #
#
# v.1.1 : make NGB-fft method run in parallel
########################################
...
pois.pd.multicore <- function(data,fdata,cpu){
	#data=std###################
	#fdata=tmp.pdata##################
	data = data/sum(data)
	fpos = fdata$y
	...
	if(pnum.check==1){
		...
		opt.fit = 0
		peak.order = rev(order(data[ppos]))
		#AP: add multi-processing
		cl <- makeCluster(cpu) #
		registerDoSNOW(cl)

		list_opt <- foreach(i = 1:pnum, .combine='comb',.export=c("comb"),.multicombine=TRUE, .init=list(list(), list()) ) %dopar% {	
		#for(i in 1:pnum){
			...
			###AP: the most time-consuming step 
			tmp.fit = nlminb(ini,fun,lower=low,upper=up,num.cluster=i)
			###########

			#if(!is.nan(tmp.fit$objective) & opt.obj>tmp.fit$objective){##
			list(tmp.fit,c(pnum=i))
		}
		stopCluster(cl)

		all_fit = data.frame()
		for (i in 1:length(list_opt[[1]])){ all_fit=rbind(all_fit,data.frame(idx=i ,obj_i=list_opt[[1]][[i]]$objective, pnum_i=list_opt[[2]][[i]] ))}
		min_opt = all_fit[which.min(all_fit$obj_i),]

		opt.obj = min_opt$objective
		opt.pnum = min_opt$pnum_i

		opt.fit = list_opt[[1]][[min_opt$idx]]
...
#mb.multicore <- function(data=data,method=method,type=type,sft=sft,sst=sst,method.pd=method.pd,d=d,pos=pos,measure=measure,i=i,gnum=gnum){
...
mb.pd <- function(data=ts.ad,method=2,type=1,sft=c(31,70),sst=c(401,500)){
...
mb.pd.multicore2 <- function(data=ts.ad,method=1,type=1,sft=c(31,70),sst=c(401,500),cpu=8){ #use multi processing in model optimization step (e.g. in method pois.pd.multicore )
	method.pd <- function(mdata,mfdata){
		if(method==2){
			return(pois.pd.multicore(data=mdata,fdata=mfdata,cpu)) ###
		}else if(method==3){
			return(tgaus.pd(data=mdata,fdata=mfdata))
		}else if(method==4){
			return(gaus.pd(data=mdata,fdata=mfdata))
	...
	for(i in 1:iterations){ # by 1st rt
		Sys.sleep(0.1)
...
mb.pd.multicore <- function(data=ts.ad,method=2,type=1,sft=c(31,70),sst=c(401,500),cpu=12){
	if(method==2){	
		export_method_function="pois.pd"
		#export_method_function="pois.pd.multicore"
	}else if(method==3){
		export_method_function="tgaus.pd"
	}else if(method==4){
		export_method_function="gaus.pd"
	}else if(method==5){
		export_method_function="gamma.pd"
	}else if(method==6){
		export_method_function="emg.pd"
	}

	method.pd <- function(mdata,mfdata){
		if(method==2){	
			return(pois.pd(data=mdata,fdata=mfdata))
			#return(pois.pd.multicore(data=mdata,fdata=mfdata))
		}else if(method==3){
			return(tgaus.pd(data=mdata,fdata=mfdata))
		}else if(method==4){
			return(gaus.pd(data=mdata,fdata=mfdata))
	...
	d = dim(data) # 1st rt by 2nd rt
	cat('# auto_GCxGC : number of cpu running ...', cpu,'\n')
	gnum = 0
	
	#AP : add multi-tread processing
	cl <- makeCluster(cpu) #not to overload your computer
	registerDoSNOW(cl)
	iterations <- d[1]
	...
	#AP : change gnum (peak count) to be unique number for each scan to allow multi-tread processing e.g. 25.010 : scan number 25, peak number 10 
	
	pos_measure <- foreach(i=1:iterations, .combine='comb',.export=c("fsd.pd",export_method_function,"hpd","comb"), .packages=c("doSNOW"),.multicombine=TRUE,.options.snow = opts, .init=list(list(), list()) ) %dopar% {
	#temp_pos=c()
	#temp_measure=c()
	#for (i in 1:d[1]){
		gnum=i
			...
			if(dlen==0){
					...
					if(length(which(tmp.pdata$y>0))!=0){
						...
						if(method!=1){
							...
							if(length(tmp.pdata$pnum)>0){
								...
								if(type==1){
									...
									tmp.sigma2 = exp(tmp$fit$par[length(tmp$fit$par)])
									tmp.mse = (tmp$fit$objective-tmp.len*log(tmp.sigma2)/2-tmp.len*log(2*pi)/2)*2*tmp.sigma2/tmp.len
									temp_measure = c(temp_measure,tmp.mse)
									rm(tmp.len)
									rm(tmp.sigma2)
									rm(tmp.mse)
								}else if(type==2)
									temp_measure = c(temp_measure,2*tmp$fit$objective+2*length(tmp$fit$par))
								else if(type==3)
									temp_measure = c(temp_measure,2*tmp$fit$objective+tmp.len*length(tmp$fit$par))
							}
							rm(tmp)
						}	
						...
						if(plen!=0){
							...
							for(j in 1:plen){
								...
								if(tpp>=trg[6] & tpp<=trg[7]){
									trg2 <- trg
									if (nchar(sub_gnum)==1){t_sub_gnum=paste('00',sub_gnum,sep='')}else if(nchar(sub_gnum)==2){t_sub_gnum=paste('0',sub_gnum,sep='')}else{t_sub_gnum=sub_gnum}
									trg2[1] <- as.numeric(paste(gnum,'.',t_sub_gnum,sep=''))
									sub_gnum=sub_gnum+1
									temp_pos = rbind(temp_pos,as.numeric(c(trg2,top,tpp,pa)))
								}
							}
						}
					}
				...
				for(j in 1:(dlen+1)){
					...
					if(j==1){
						...
						trg = c(gnum,i,gpos[dpos[j-1]+1],gpos[dpos[j]],(sft[1]-1+i),(sst[1]-1+gpos[dpos[j-1]+1]),(sst[1]-1+gpos[dpos[j]]))
					}else{
						trg = c(gnum,i,gpos[dpos[j-1]+1],gpos[glen],(sft[1]-1+i),(sst[1]-1+gpos[dpos[j-1]+1]),(sst[1]-1+gpos[glen]))
					}
					trg = as.numeric(trg)
	...
	close(pb)####################
	stopCluster(cl)
	list_pos <- pos_measure[[1]][lengths(pos_measure[[1]]) != 0]
	...
	if(length(list_pos)==0){
		...
		for (i in 1:length(list_pos)){
			pos=rbind(pos,list_pos[[i]])
		}

		list_measure <- pos_measure[[2]][lengths(pos_measure[[2]]) != 0]

		if (length(list_measure)==0 ){
			measure=c(0)
		}else{		
			measure=c()	
			for (i in 1:length(list_measure)){
				measure=c(measure,list_measure[[i]])
			}
		}
...
ptable.wo.merg <- function(data=g.ad$nd,subt1=subrt1,subt2=subrt2,p1=4,p2=0.005,pmin1=0,pmin2=0.0,sic.name="sicdata"){
	len = dim(data)[1]
	rlt = c()
	for(i in 1:len){
		td = as.numeric(data[i,])
		...
		rlt = rbind(rlt,c(i,tpos,td[3],td[9],td[11],tmp.ms)) #AS: add ,tmp.ms to list
	}
	rlt = data.frame(rlt)
	dimnames(rlt)[[2]][1:6] = c("id","t1","t2","tid1","tid2","area")
	rlt
}

ptable <- function(data=ms.ad.odd100.y,subt1=subrt1,subt2=subrt2,p1=4,p2=0.005,pmin1=0,pmin2=0.0,sic.name="sicdata"){
	len = dim(data)[1]
	rlt = c()
	for(i in 1:len){
		td = as.numeric(data[i,])
...
msPeakG <- function(
	...
	#gp.rg select the interesting subregion
	...
	if(merge){
		...
		# peak table
		ptab.odd = ptable(ms.ad.odd,subt1=subrt1,subt2=subrt2,sic.name=sic.name,p1=x$p1,p2=x$p2,pmin1=x$pmin1,pmin2=x$pmin2)
		...
		if(verbose)
			cat("# msPeakG #: Create the peak table ...\n")
		# peak table
		ptab.odd = ptable.wo.merg(gs.ad.odd$nd,subt1=subrt1,subt2=subrt2,sic.name=sic.name,p1=x$p1,p2=x$p2,pmin1=x$pmin1,pmin2=x$pmin2)
...
#########
#########
# msPeakG, modified: multi processors
#########
msPeakG.multicore <- function(
			x # output from "find.sub"
			,zcv=.5 # the cut-off of the z-prob for peak region detection
			,method=1 # 1=first derivative, 2=poisson, 3=tgaussian, 4=gaussian, 5=gamma, 6=emg
			,cv=0.95 # the cut-off of the mass spectrum similarity score for peak merging
			...
			,imax=10 # maximum intensity
			,cpu=12
			,parallel_mode=1 # mode1 : do parallel process at retention time step (faster when the number of peak candidates are low in each retention time step), mode2: do parallel process at function optimization step (faster when the data contain many peak candidates (high noise)). Both methods yield the same result.
){
	if(is.na(out.fname)){
		out.fname = gsub("-","",Sys.Date())
	}
	...
	if(ngb){
		...
		if(verbose)
			cat("# msPeakG #: Step III - Peak peaking using ",method.lab[method]," and grouping ...\n")
			cat("# msPeakG #: Using multi-processors : ",cpu," cpu cores \n")
		# peak finding and dgrouping
		if(parallel_mode==1){
			gfs.ad.odd = mb.pd.multicore(ts.ad.odd,method,sft=range(subrt1),sst=range(subrt2),cpu=cpu)$pos
		}
		else if (parallel_mode==2){	
			gfs.ad.odd = mb.pd.multicore2(ts.ad.odd,method,sft=range(subrt1),sst=range(subrt2),cpu=cpu)$pos
		}
	...
	if(merge){
		...
		if(verbose)
			cat("# msPeakG #: Create the peak table ...\n")
		# peak table 
		# AS: fix by pass p1,p2,pmin parameters to ptable method to avoid using default parameters 
		ptab.odd = ptable(ms.ad.odd,subt1=subrt1,subt2=subrt2,sic.name=sic.name,p1=x$p1,p2=x$p2,pmin1=x$pmin1,pmin2=x$pmin2)
...
find.subG2 <- function(
	...
	cat('# msPeakG : 1st RT should be in range ',1,' to ',(tic.size[1]),'\n')
	cat('# msPeakG : 2st RT should be in range ',1,' to ',(tic.size[2]),'\n')
	#cat('# msPeakG : 1st RT should be in range ',(pmin1/p1),' to ',(tic.size[1]+(pmin1/p1)),'\n')
	#cat('# msPeakG : 2st RT should be in range ',(pmin2/p2),' to ',(tic.size[2]+(pmin2/p2)),'\n')

	if (pmin1 >0){ cat('# msPeakG : pmin1 was specified with value ',pmin1,' sec or ',(pmin1/p1),' 1st RT time\n')
		cat('# msPeakG : total scan time is in between ',pmin1,' to ',floor((tic.size[1]+(pmin1/p1))*p1),' sec\n')
		cat('# msPeakG : so, 1st RT should be in range ',(pmin1/p1),' to ',(tic.size[1]+(pmin1/p1)),'\n')

		if (subrt1[1] < (pmin1/p1)){
			cat('# msPeakG : please put starting 1stRT (',subrt1[1],') > minimum 1stRT that you specified (',pmin1,' sec) or higher than ',floor(pmin1/p1), '\n')
			#return('fail')
		}else{
			subrt1[1]=subrt1[1]-floor(pmin1/p1)
		}
	}else if (pmin2 >0){ cat('# msPeakG : pmin2 was specified with value ',pmin2,' sec or ',(pmin2/p2),' 1st RT time\n')
		cat('# msPeakG : modulation time is  ',p1,' sec\n')
		cat('# msPeakG : so, 2st RT should be in range ',(pmin2/p2),' to ',(tic.size[2]+(pmin2/p2)),'\n')

		if (subrt2[1] < (pmin2/p2)){
			cat('# msPeakG : please put starting 2ndRT (',subrt2[1],') > minimum 2ndRT that you specified (',pmin2,' sec) or higher than ',floor(pmin2/p2), '\n')
			#return('fail')
		}else{
			subrt2[1]=floor(pmin2/p2)+subrt2[1]
		}	
	}

	#AS: resolved
	xr = (c(range(subrt1))-1)*p1 
	yr = (c(range(subrt2))-1)*p2
	xr[1] = xr[1]+pmin1
	yr[1] = yr[1]+pmin2
	...
	if(plot){
		...
		sub.contourG(tic.fname=tic.fname,
		sub1=subrt1,
		sub2=subrt2,
		col=col,
		p1=p1,
