...
## Edited vertion of function create.factorlist.table in ERah package
## change N.groups[-which(N.groups==0)] to N.groups[N.groups!=0)]
create.factorlist.table <- function(object){
	...
	N.groups <- N.groups[N.groups!=0] #AP: modified from N.groups[-which(N.groups==0)]
	...
	#AP edited to avoid error Error "in which(spectra != 0) :  (list) object cannot be coerced to type 'double' 
